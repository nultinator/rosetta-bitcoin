...
func (s *ConstructionAPIService) estimateSize(operations []*types.Operation) float64 {
	size := ycash.TransactionOverhead
	for _, operation := range operations {
		switch operation.Type {
		case ycash.InputOpType:
			size += ycash.InputSize
		case ycash.OutputOpType:
			size += ycash.OutputOverhead
			addr, err := btcutil.DecodeAddress(operation.Account.Address, s.config.Params)
			if err != nil {
				size += ycash.P2PKHScriptPubkeySize
				continue
			}
...
func (s *ConstructionAPIService) ConstructionMetadata(
	...
	if feePerKB < ycash.MinFeeRate {
		feePerKB = ycash.MinFeeRate
	}

	// Calculated the estimated fee in Satoshis
	satoshisPerB := (feePerKB * float64(ycash.SatoshisInBitcoin)) / bytesInKb
...
func (s *ConstructionAPIService) ConstructionCombine(
	...
	for i := range tx.TxIn {
		...
		class, _, err := ycash.ParseSingleAddress(s.config.Params, decodedScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) parseUnsignedTransaction(
	...
	for i, output := range tx.TxOut {
		networkIndex := int64(i)
		_, addr, err := ycash.ParseSingleAddress(s.config.Params, output.PkScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) parseSignedTransaction(
	...
	for i, input := range tx.TxIn {
		...
		_, addr, err := ycash.ParseSingleAddress(s.config.Params, pkScript.Script())
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionSubmit(
	...
	if err != nil {
		return nil, wrapErr(ErrYcashd, fmt.Errorf("%w unable to submit transaction", err))
	}
